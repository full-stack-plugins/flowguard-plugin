...
def main():
    ...
    try:
        ...
        return 0  # 未初始化：静默
    lines = [f"[flowgate] 项目 {project['project']} | current_feature: {project.get('current_feature') or '未设定'}"]
    lines.append("[flowgate] 项目级阶段: " + ", ".join(
    ...
    for fid in (project.get("features") or {}):
        ...
        lines.append(f"[flowgate] 功能 {fid}({f.get('status')}): {sts}")
    print("\n".join(lines))
    return 0
