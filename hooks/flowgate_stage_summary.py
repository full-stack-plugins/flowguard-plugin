...
from flowgate_lib import state  # noqa: E402
    ...
    if fid:
        try:
            ...
            if f.get("status") == "active":
                if nxt:
                    print(f"[flowgate] 功能 {fid} 下一步: 阶段 {nxt}（/flowgate-next）")
                else:
                    print(f"[flowgate] 功能 {fid} 全阶段已收敛，可 /flowgate-feature done {fid}")
        except Exception:
            pass
    pending = [s for s in ("architecture", "standards", "release")
               if project["stages"][s]["status"] not in OK]
    if pending:
        print(f"[flowgate] 项目级待推进: {', '.join(pending)}（/flowgate-next --stage {pending[0]}）")
    active = [k for k, v in (project.get("features") or {}).items() if v.get("status") == "active"]
    if not active and not pending:
        print("[flowgate] 全部收敛，可进入部署交付（/flowgate-next --stage release）")
    return 0
