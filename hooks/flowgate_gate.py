...
from flowgate_lib import gate, registry  # noqa: E402
...
def main():
    try:
        ...
        print("WARNING: flowgate_gate 收到非法 stdin，放行", file=sys.stderr)
        return 0
    ...
    try:
        ...
        print(f"WARNING: flowgate_gate 异常放行: {e}", file=sys.stderr)
        return 0
    if res["allowed"]:
        return 0
    env = res["envelope"]
    ...
    print(f"[flowgate] code={env['code']}", file=sys.stderr)
    return 2
