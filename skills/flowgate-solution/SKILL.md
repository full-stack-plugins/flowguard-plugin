---
name: flowgate-solution
license: Apache-2.0
description: 技术方案阶段。当用户为某功能定实现方案、接口契约或风险预案时使用。不要用它做全项目架构决策（flowgate-architecture）。
compatibility: 需要项目内已运行 /flowgate-init 且存在 current_feature（项目级阶段除外）；阶段推进经由编排核 CLI。
---

# flowgate-solution —— 产出功能技术方案 03-solution.md：实现选型 / 接口契约 / 风险清单。
...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" instructions 03-solution --json  # 本阶段 context/rules/模板/依赖/Tier2 技能
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" validate --json            # 产物自检
```

## 产物

写入 `.flowgate/` 下 `03-solution` 对应产物，模板见 `references/templates/03-solution.md`。
