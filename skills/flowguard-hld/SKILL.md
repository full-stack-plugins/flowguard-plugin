...
name: flowguard-hld
license: Apache-2.0
description: 兼容模式，仅当项目已有旧 .flowguard 十阶段状态或用户明确要求旧流程时使用。概要设计阶段（薄）。当用户要划分模块/服务、描述交互与数据流时使用。不要用它写类级明细（flowguard-lld）。
...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowguard_state.py" instructions 05-hld --json  # 本阶段 context/rules/模板/依赖/Tier2 技能
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowguard_state.py" validate --json            # 产物自检
```

## 产物

写入 `.flowguard/` 下 `05-hld` 对应产物，模板见 `references/templates/05-hld.md`。
