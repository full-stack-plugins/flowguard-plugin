...
# flowgate-release —— 产出发布清单 10-release.md：版本/校验和/回滚方案/证据。
...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" instructions 10-release --json  # 本阶段 context/rules/模板/依赖/Tier2 技能
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" validate --json            # 产物自检
```

## 产物

写入 `.flowgate/` 下 `10-release` 对应产物，模板见 `references/templates/10-release.md`。
