...
# flowgate-review —— 产出证据化代码审查报告 08-review.md：发现项（证据）+ 结论（fix/wontfix/deferred）。
...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" instructions 08-review --json  # 本阶段 context/rules/模板/依赖/Tier2 技能
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" validate --json            # 产物自检
```

## 产物

写入 `.flowgate/` 下 `08-review` 对应产物，模板见 `references/templates/08-review.md`。
