...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowguard_state.py" instructions 02-architecture --json  # 本阶段 context/rules/模板/依赖/Tier2 技能
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowguard_state.py" validate --json            # 产物自检
```

## 产物

写入 `.flowguard/` 下 `02-architecture` 对应产物，模板见 `references/templates/02-architecture.md`。
