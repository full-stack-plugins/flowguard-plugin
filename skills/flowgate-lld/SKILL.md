...
# flowgate-lld —— 产出详细设计 06-lld.md：类/表/接口明细与异常边界。
...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" instructions 06-lld --json  # 本阶段 context/rules/模板/依赖/Tier2 技能
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" validate --json            # 产物自检
```

## 产物

写入 `.flowgate/` 下 `06-lld` 对应产物，模板见 `references/templates/06-lld.md`。
