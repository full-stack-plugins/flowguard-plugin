...
# flowgate-testcases —— 产出测试用例 04-testcases.md：用例 + 追溯矩阵（REQ↔用例↔测试文件）。
...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" instructions 04-testcases --json  # 本阶段 context/rules/模板/依赖/Tier2 技能
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/flowgate_state.py" validate --json            # 产物自检
```

## 产物

写入 `.flowgate/` 下 `04-testcases` 对应产物，模板见 `references/templates/04-testcases.md`。
...
validate 无 ERROR：REQ 全覆盖、测试文件全部存在。验收由用户执行 /flowgate-advance 写入 accepted。
